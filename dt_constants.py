...
class Game_States(Enum):
    eDisableBtns    = auto()
    ePlayer         = auto()    #prompt for number of players
    eLevel          = auto()    #prompt for level
    eReadyPlayer    = auto()    #prompt for next player
    eIdle           = auto()    #wait for player to press button
    eBazaar         = auto()    #player is at the bazaar shopping
    eBuy            = auto()    #player is buying current item at Bazaar


#Queue Messages
class Queue_Msgs(Enum):
    eNoButton = auto()
    eDisplaySetup1 = auto() #display prompt for n Players
    eDisplaySetup2 = auto() #display prompt for level
    eReadyPlayer = auto()   #display ready player prompt

    eOneBtn = auto()        #handle buttons
    eTwoBtn = auto()
    eThreeBtn = auto()
    eFourBtn = auto()
    eBazaarBtn = auto()
